#' Fill Gaps Using Beers Method Interpolation
#'
...
interpolate_beers <- function(input_df, groups, year, values, method = "ordinary") {
  ...
  # Check if specified columns exist in the input_df
  required_columns <- c(groups, year, values)
  missing_columns <- setdiff(required_columns, names(input_df))
  if (length(missing_columns) > 0) {
    stop("Missing columns in input data: ", paste(missing_columns, collapse = ", "))
  }
