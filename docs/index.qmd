...
gert::git_add_all()
gert::git_commit("Corrige workflow do GitHub Actions para execução correta")
gert::git_push()
...
This step integrates additional labels by merging auxiliary datasets with the main dataset. It uses ISO codes, educational levels, cohort categories, and gender information.
The default label correspondences included in the package are shown below. They can be modified with external sources if needed.
...
# Step 5: Preparing Final Output Data for Export
...
### Convert to Arrays and Add Descriptions
...
# Optional Step: Download and Update input data
...
```{r}
#| eval: false
# Define the Python executable path and output file location
