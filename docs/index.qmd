...
# deps <- pkgdepends::new_pkg_deps("tsimonato/gtapssp")
# deps$solve()
# deps$get_solution()
# 
# remotes::install_github("USDA-ERS/MTED-HARr", force = TRUE)
# 
# write.csv(ssp_data, file = "out.csv")
...
The package provides optimized and user-friendly functions to download SSP data, interpolate data using spline and beers methods. The *gtapssp* functions is accompanied by detailed [manual](https://github.com/tsimonato/gtapssp/raw/master/docs/gtapssp_0.0.0.9000.pdf), you can also access this manual by pressing `F1` on the function name in [RStudio](https://posit.co/download/rstudio-desktop/).
...
1.  Download the R installer from the [Comprehensive R Archive Network (CRAN)](https://cran.r-project.org/).
...
::: callout-important
Python installation is **only required** if you plan to download other versions of SSPs datasets from the IIASA. The `gtapssp` package includes a built-in default dataset, so installing Python is optional unless you wish to access the latest or alternative datasets from IIASA's API (developed in Python).
:::
...
#| eval: true
#| message: false
ssp_data <- read.csv("out.csv")
DT::datatable(ssp_data[1:40,])
```
...
data_map  <- ssp_data |> 
  dplyr::mutate(iso_a3 = ISO) |> 
  dplyr::filter(MOD == "IIASA-WiC POP 2023",
                SCE == "SSP1",
                GND == "FEML",
                AGE == "P65UP",
                YRS == "Y1950")
# Example: Plot a map using the `plot_map` function
gtaptools::plot_map(
  input_data = data_map,       # Your data frame
...
Below is a spatial representation of GDP per capita projections. The map below shows GDP per capita in PPP (2017 USD) for the year 2100 under the SSP1 scenario. The data comes from the OECD ENV-Growth 2023 dataset.
...
gtaptools::plot_map(
  ...
  value_var = "value",    # Replace with the column name for numeric values to plot
  colors = "viridis",
  legend_title = "(PPP, USD 2017)",        # Replace with a title for your legend
)
```
...
#| eval: true
#| message: false
DT::datatable(gtapssp::iiasa_raw$data[1:40,], height = 300, options = list(
                scrollX = TRUE,
                scrollY = "300px",
                scrollCollapse = TRUE
              ))
...
GDPI <- gtap_ssp |> 
  tidyr::complete(MOD, VAR, SCE, ISO, YRS, fill = list(value = 0)) |> # Ensure all combinations exist with default values
...
This tutorial and the development of the gtapssp package were supported by the **[GTAP Team](https://www.gtap.agecon.purdue.edu/default.asp)**, with special thanks to [**Dr. Dominique van der Mensbrugghe**](https://www.gtap.agecon.purdue.edu/network/member_display.asp?UserID=415), [**Dr. Angel Aguiar**](https://www.gtap.agecon.purdue.edu/network/member_display.asp?UserID=6446), and [**Dr. Erwin Corong**](https://www.gtap.agecon.purdue.edu/network/member_display.asp?UserID=2958) for their guidance and expertise.
